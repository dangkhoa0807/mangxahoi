  ...
  >
    <div class="flex flex-col gap-4">
      <div class="flex flex-col items-center w-full gap-4 my-5">
        ...
        ></Icon>
        <p>Bạn có chắc muốn gỡ bài viết này?</p>
      </div>

      <div v-if="showPenaltyOptions" class="border-t pt-4">
        <div class="font-semibold mb-3">Xử phạt người đăng</div>
...
async function handleRemove() {
  try {
    if (props.showPenaltyOptions) {
      ...
      // Xử lý xóa bài viết thông thường
      ...
      if (removeResponse.value?.code !== 200) {
        throw new Error(
          removeResponse.value?.message || "Không thể xóa bài viết"
        );
      }
    }

    toast.success("Đã gỡ bài viết thành công");
