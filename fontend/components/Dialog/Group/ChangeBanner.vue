  ...
  >
    <div class="flex flex-col items-center">
      ...
      >
        ...
        >
          <Icon name="tabler:photo" class="text-blue-500 text-3xl" />
        </div>
      ...
      <div class="flex justify-end gap-2 mt-4 w-full">
        <Button
          ...
          class="p-button-text"
        />
        <Button label="Lưu thay đổi" :loading="saving" @click="updateBanner" />
      </div>
    </div>
  </Dialog>
</template>
...
async function updateBanner() {
  ...
  try {
    ...
    const { data: response } = await useAPI<ApiResponse>(
      `/api/group/${props.groupId}/banner`,
      {
        method: "PUT",
        body: formData,
      }
    ...
    if (response.value?.code === 200) {
      toast.success("Cập nhật ảnh bìa nhóm thành công");
      emit("updated");
      model.value = false;
    } else {
      toast.error(response.value?.message || "Cập nhật ảnh bìa nhóm thất bại");
    }
  } catch (error) {
    toast.error("Đã xảy ra lỗi khi cập nhật ảnh bìa nhóm");
  } finally {
    saving.value = false;
  }
}
